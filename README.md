...
Всё соберётся и запустится. Откройте **http://localhost:8044** в браузере.

**Что поднимается:**

| Сервис | Порт | Описание |
|--------|------|----------|
| Приложение | `8044` | Фронтенд + API |
...
# Запустить сервер
poetry run uvicorn app.main:app --host 0.0.0.0 --port 8044 --reload
```
...
В продакшене фронтенд раздаётся самим бэкендом на порту `8044`. В режиме `dev` используйте Vite на порту `5173` — он будет проксировать API-запросы на бэкенд.
