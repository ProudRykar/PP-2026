...
└──────────────┘                           └──────────┬───────────────┘
            ...
            │ (DATABASE_PORT)   │              │ (MINIO_API_PORT) │   │ (движки: poll)   │
            └──────────────────┘              └──────────────────┘   └──────────────────┘
                                                                           │
                                                 ┌─────────────────────────┼──────────────┐
                                                 ▼                         ▼              ▼
...
Всё соберётся и запустится. Откройте **http://localhost:8044** (или ваш `APP_PORT`) в браузере.

**Что поднимается:**

| Сервис | Переменная | Порт по умолч. | Описание |
|--------|-----------|----------------|----------|
| Приложение | `APP_PORT` | `8044` | Фронтенд + API |
| PostgreSQL | `DATABASE_PORT` | `5432` | База данных |
| MinIO API | `MINIO_API_PORT` | `9000` | S3-хранилище (файлы) |
| MinIO Console | `MINIO_CONSOLE_PORT` | `9001` | Веб-консоль MinIO |

Все порты можно изменить через `.env` или переменные окружения при запуске. Например:

```bash
APP_PORT=8080 DATABASE_PORT=5433 podman compose up -d --build
```
...
# Остановить, удалить контейнеры и СТЕРЕТЬ все данные (БД, файлы)
...
# Запустить сервер
poetry run uvicorn app.main:app --host ${APP_HOST:-0.0.0.0} --port ${APP_PORT:-8044} --reload
```
...
# или
npm run dev      # режим разработки (порт 5173 с прокси на 8044)
```

В продакшене фронтенд раздаётся самим бэкендом на порту `APP_PORT` (`8044` по умолчанию). В режиме `dev` используйте Vite на порту `5173` — он будет проксировать API-запросы на бэкенд.
...
1. Откройте **http://localhost:8044**
