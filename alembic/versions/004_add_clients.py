...
from _alembic_helpers import table_exists, index_exists
...
def upgrade() -> None:
    ...
    if not table_exists("client_channels"):
        op.create_table(
            "client_channels",
            sa.Column("id", sa.String(), nullable=False),
    ...
    if not index_exists("client_channels", "ix_client_channels_client_id"):
        op.create_index(
            "ix_client_channels_client_id", "client_channels", ["client_id"]
        )

    if not index_exists("client_channels", "ix_client_channels_channel_external_id"):
        op.create_index(
